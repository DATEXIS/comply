# FOR RAY ==  2.38.0
# /opt/conda/lib/python3.10/site-packages/ray/rllib/utils/serialization.py
import base64
...
from gym_medical.envs.doctor_sim import TextSpace
...
def gym_space_to_dict(space: gym.spaces.Space) -> Dict:
    ...
    def _medtext(sp: "TextSpace") -> Dict:
        return {
            "space": "medtext",
            "min_length": sp.length,
            "max_length": sp.vocab_size,
        }
...
def gym_space_from_dict(d: Dict) -> gym.spaces.Space:
    ...
    def _medtext(d: Dict) -> "TextSpace":
        #return TextSpace(**d)
        return TextSpace(length=d['min_length'], vocab_size=d['max_length'])
